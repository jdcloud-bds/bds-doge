# bds-doge
## 介绍
bds-doge 是开源项目区块链数据服务（BDS）的其中一个独立模块 - 提供全节点服务。

*bds-doge* 基于 [dogecoin/dogecoin](https://github.com/dogecoin/dogecoin) 的 v1.14 版本进行二次开发，支持直接将新增的区块数据往消息中间件服务 kafka 发送，方便上游的服务来订阅消费。
...
```
   /dogecoin/src/dogecoind -kafka -kafkaproxyhost=<kafka 代理的ip地址> -kafkaproxyport=<kafka 代理的访问端口，默认是 8082> -kafkatopic=doge -datadir=<数据目录> -rpcuser=<账号> -rpcpassword=<密码>
```
