...
static UniValue sendblock(const JSONRPCRequest& request)
{
    ...
    if (IsArgSet("-kafka")) {
        ...
        int ret = post(GetArg("-kafkaproxyhost", "localhost"), GetArg("-kafkaproxyport", "8082"), "/topics/" + GetArg("-kafkatopic", "test"), "{\"records\":[{\"value\":" + result.write() + "}]}", response_data);
        if (ret != 0) {
...
static UniValue sendbatchblock(const JSONRPCRequest& request)
{
    ...
    if (startHeight < 0 || endHeight > chainActive.Height() || startHeight > endHeight || endHeight - startHeight > 10000){
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Block height out of range");
    }
    for(int i = startHeight; i <= endHeight;i++){
        ...
        if (IsArgSet("-kafka")) {
            std::string response_data;
            int ret = post(GetArg("-kafkaproxyhost", "localhost"), GetArg("-kafkaproxyport", "8082"), "/topics/" + GetArg("-kafkatopic", "test"), "{\"records\":[{\"value\":" + r.write() + "}]}", response_data);
            if (ret != 0) {
                std::cout << "error_code:" << ret << std::endl;
                std::cout << "error_message:" << response_data << std::endl;
            }
