...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-kafka", _("Bitcoin support kafka"));
    strUsage += HelpMessageOpt("-kafkaproxyhost=<ip>", _("Kafka proxy host"));
    strUsage += HelpMessageOpt("-kafkaproxyport=<port>",_("Kafka proxy port"));
    strUsage += HelpMessageOpt("-kafkatopic=<name>",_("Kafka topic name"));
...
bool AppInitParameterInteraction()
{
    ...
    if (IsArgSet("-kafka")) {
        size_t kafkaProxyHost = GetArg("-kafkaproxyhost","").size();
        size_t kafkaProxyPort = GetArg("-kafkaproxyport","").size();
        size_t kafkaTopic = GetArg("-kafkatopic","").size();
        if (kafkaProxyHost == 0) {
