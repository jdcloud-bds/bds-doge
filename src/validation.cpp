...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
    ...
    NotifyHeaderTip();
    static int kafkaHeightrRange = chainActive.Height()+1;//kafkaHeightrRange must be initialized before ActivateBestChain()
    ...
    if (IsArgSet("-kafka")) {
        myPrintBlockOrderByHeight(kafkaHeightrRange);
    }
    return true;
}
...
UniValue myBlockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("stripped_size", (int)::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS));
...
{
    entry.pushKV("tx_id", tx.GetHash().GetHex());
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (tx.IsCoinBase())
            ...
            in.pushKV("tx_id_origin", txin.prevout.hash.GetHex());
            in.pushKV("vout_num_origin", (int64_t)txin.prevout.n);
            ...
            if (!tx.vin[i].scriptWitness.IsNull()) {
                ...
                in.pushKV("tx_in_witness", txinwitness);
            }
        }
...
UniValue myGetBlock(const int height){
    ...
    const CBlock block = myGetBlockChecked(pblockindex);
...
std::vector<UniValue> myGetBlockbatch(const int heightStart,const int heightEnd){
    ...
    if (heightStart < 0 || heightEnd > chainActive.Height()||heightStart<heightEnd||heightEnd-heightStart>10000){
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Block height out of range");
    }
...
void myPrintBlockOrderByHeight(int &kafkaHeightrRange){
    ...
    if(kafkaHeightrRange<=chainActive.Height()){
        for(int i=kafkaHeightrRange;i<=chainActive.Height();i++){
            ...
            int ret = post(GetArg("-kafkaproxyhost", "localhost"), GetArg("-kafkaproxyport", "8082"), "/topics/" + GetArg("-kafkatopic", "test"), "{\"records\":[{\"value\":" + myGetBlock(i).write() + "}]}", response_data);
            if (ret != 0) {
